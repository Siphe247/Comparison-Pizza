...
# Player values. The ones with 100s are divided by 100 to accommodate circle radii. The 10s are divided by 10 for the same reason.
raphinha_values=[0.66,0.43,1.08,0.56,0.45,1.01,0.52,0.97]
yamal_values=[0.37,0.39,0.75,0.32,0.37,0.69,0.32,0.69]
salah_values=[0.68,0.46,1.14,0.66,0.38,1.04,0.47,0.85]
dembélé_values=[0.82,0.33,1.15,0.78,0.38,1.15,0.75,1.13]
...
# Shift the axes position lower by adjusting y0
pos = ax.get_position()
new_pos = [pos.x0, pos.y0 - 0.05, pos.width, pos.height]
ax.set_position(new_pos)

# Plotting the radar
rings_inner = radar.draw_circles(ax=ax, facecolor="#000000", edgecolor="#CCCCCC")
...
range_labels = radar.draw_range_labels(ax=ax, fontsize=20, color='white')
param_labels = radar.draw_param_labels(ax=ax, fontsize=20, color='white')


# Add title
fig_text(
    0.515, 1.03, "<Raphinha> vs <Lamine Yamal> vs <Mohamed Salah> vs <Ousmane Dembélé>",
    size=16, fig=fig,
...
# Add subtitle
fig_text(
    0.515, 1.00, "2024/25 Season Per 90 Minutes in all competitions. 90 minute games played: <51.7>, <57.1>, <49.1>, <42.7>",
    size=10, fig=fig,
    highlight_textprops=[{"color": "#01CCBF"}, {"color": '#4361EE'}, {"color": '#FF6B6B'},{'color': '#FFD166'}],
    ha="center", color="white"
)
...
# Add Raphinha image
ax1 = fig.add_axes([0.05,0.80,0.10,0.25])
ax1.axis('off')
...
ax2=fig.add_axes([0.15,0.80,0.09,0.25])
ax2.axis('off')
...
#Add Yamal image
ax3 = fig.add_axes([0.25,0.80,0.10,0.25])
ax3.axis('off')
...
#Add Barcelona image
ax4=fig.add_axes([0.35,0.80,0.09,0.25])
ax4.axis('off')
img=Image.open('/Users/siphuvuyomngxunyeni/Downloads/Football Analytics/FC Barcelona club badge.png')
ax4.imshow(img)
...
#Add Salah image
ax5=fig.add_axes([0.58,0.80,0.10,0.25])
ax5.axis('off')
...
#Add Liverpool image
ax6=fig.add_axes([0.68,0.80,0.09,0.25])
ax6.axis('off')
...
#Add Dembélé image
ax7=fig.add_axes([0.78,0.80,0.10,0.25])
ax7.axis('off')
...
#Add PSG image
ax8=fig.add_axes([0.88,0.80,0.09,0.25])
ax8.axis('off')
...
plt.savefig('/Users/siphuvuyomngxunyeni/Downloads/Football Analytics/Raphinha vs Yamal vs Salah vs Dembélé 24-25 season.png', dpi=300, bbox_inches='tight', facecolor=fig.get_facecolor())
plt.show()
